...
/**
 * --- ОНОВЛЕНА ФУНКЦІЯ ---
 * Додано логіку для групування паркомісць.
 */
function fok_filter_properties_ajax_handler() {
    ...
    foreach ($properties_by_section as $section_id => $properties) {
        ...
        foreach ($parking_items as $spot) {
            ...
            if ($status_slug === 'vilno') $available_parking_count++;
            ...
            $final_parking_items[] = [
                 ...
                 'rooms' => 0, 'has_discount' => (float)get_post_meta($spot->ID, 'fok_property_discount_percent', true) > 0,
            ];
        }
...
function fok_autogenerate_property_title_on_save( $post_id ) {
    // Перевірки, щоб уникнути виконання при автозбереженні, або якщо у користувача немає прав
    if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) return;
    if ( ! current_user_can( 'edit_post', $post_id ) ) return;
    
    // Визначаємо дозволені типи постів
    $allowed_post_types = ['apartment', 'commercial_property', 'parking_space', 'storeroom'];
    $post_type = get_post_type($post_id);

    // Виконуємо код тільки для наших типів нерухомості
    if ( !in_array($post_type, $allowed_post_types) ) return;

    // Отримуємо номер об'єкта з даних, що надсилаються формою
    // Використовуємо $_POST, оскільки хук save_post спрацьовує до того, як MetaBox оновить мета-дані
    $property_number = isset( $_POST['fok_property_number'] ) ? sanitize_text_field( $_POST['fok_property_number'] ) : '';

    // Якщо номер не вказаний, нічого не робимо
    if ( empty($property_number) ) {
        return;
    }
    
    // Масив з назвами для заголовків
    ...
    // Отримуємо відповідну назву типу
    $type_name = $type_names_for_title[$post_type] ?? __('Об\'єкт', 'okbi-apartments');

    // Формуємо новий заголовок
    $new_title = $type_name . ' №' . $property_number;

    // Отримуємо поточні дані поста, щоб перевірити, чи відрізняється заголовок
    $current_post = get_post($post_id);
    if ($current_post->post_title === $new_title) {
        return; // Якщо заголовок вже правильний, нічого не робимо
    }

    // Тимчасово відключаємо наш хук, щоб уникнути нескінченного циклу
    remove_action( 'save_post', 'fok_autogenerate_property_title_on_save', 20 );

    // Оновлюємо пост з новим заголовком
    wp_update_post([
        'ID'         => $post_id,
        'post_title' => $new_title,
        'post_name'  => sanitize_title($new_title) // Також оновлюємо slug (постійне посилання)
    ]);

    // Повертаємо наш хук на місце
