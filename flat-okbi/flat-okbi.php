...
function fok_enqueue_admin_scripts( $hook_suffix ) {
    ...
    if ( 'section' === $current_screen->post_type ) {
        ...
        wp_enqueue_script( 'fok-admin-groups-script', plugin_dir_url( __FILE__ ) . 'assets/js/admin-groups.js', ['jquery', 'jquery-ui-sortable'], time(), true );
...
function fok_ajax_get_properties_for_floor_json() {
    ...
    $property_types = ['apartment', 'commercial_property', 'storeroom']; // Паркомісця зазвичай не на планах поверхів
    $properties_query = new WP_Query([
        'post_type'      => $property_types,
        'posts_per_page' => -1,
        'meta_query'     => [
