...
/**
 * Завантажує файли локалізації плагіна.
 *
 * Переконується, що плагін може бути перекладено, завантажуючи
 * файли перекладу з папки /languages/.
 */
function fok_load_textdomain() {
...
add_action( 'plugins_loaded', 'fok_load_textdomain' );
...
function fok_render_viewer_shortcode() {
    ...
    ob_start();
    ?>
    <div id="fok-viewer-fullscreen-container" style="--fok-accent-color: <?php echo esc_attr($accent_color); ?>; --fok-accent-color-rgb: <?php echo esc_attr($rgb_color); ?>;">
        <header class="fok-viewer-header">
            ...
            <div class="fok-header-actions">
                <button id="fok-mobile-filter-trigger" title="<?php esc_attr_e('Фільтри', 'okbi-apartments'); ?>">
                    <?php echo fok_get_icon('filter'); ?>
                </button>
                <?php 
                $phone_number = $options['sales_phone'] ?? '';
                if ( !empty($phone_number) ) {
                    $formatted_phone_link = 'tel:' . preg_replace('/[^\d+]/', '', $phone_number);
                    echo '<a href="' . esc_attr($formatted_phone_link) . '" class="fok-header-phone-desktop fok-phone-detailed">' . fok_get_icon('phone') . '<div class="fok-phone-text-wrapper"><span>Консультація</span><strong>' . esc_html($phone_number) . '</strong></div></a>';
                    echo '<a href="' . esc_attr($formatted_phone_link) . '" class="fok-header-phone-mobile">' . fok_get_icon('phone') . '</a>';
                }
                ?>
                <button id="fok-viewer-close" title="<?php esc_attr_e('Закрити', 'okbi-apartments'); ?>"><?php echo fok_get_icon('close'); ?></button>
            </div>
        </header>
            ...
            <div id="fok-list-mode" class="active">
                <div class="fok-list-container">
                    <aside class="fok-list-sidebar">
                        <button id="fok-sidebar-close" title="<?php esc_attr_e('Закрити', 'okbi-apartments'); ?>"><?php echo fok_get_icon('close'); ?></button>
                        <h3><?php echo fok_get_icon('search'); ?><span><?php _e('Параметри пошуку', 'okbi-apartments'); ?></span></h3>
                    ...
                    <aside id="fok-details-panel">
                        <button id="fok-panel-close" title="<?php esc_attr_e('Закрити', 'okbi-apartments'); ?>"><?php echo fok_get_icon('close'); ?></button>
...
function fok_enqueue_frontend_assets() {
    ...
    // Передаємо SVG іконки у скрипт
    wp_localize_script( 'fok-frontend-script', 'fok_icons', [
        'parking' => fok_get_icon('parking'),
    ]);
    // === КІНЕЦЬ ЗМІН ===
}
...
function fok_filter_properties_ajax_handler() {
    ...
    $properties_query = new WP_Query([
        ...
        'fields'         => 'ids', // Головна оптимізація: отримуємо тільки ID
    ]);
    ...
    if ( ! empty($properties_query->posts) ) {
        foreach ($properties_query->posts as $property_id) {
            ...
            if ($section_id && isset($sections_data[$section_id])) {
                // Зберігаємо ID, а не цілий об'єкт поста
    ...
    // Тепер сортуємо масив ID
        ...
        foreach ($parking_items as $spot_id) { // Змінено $spot на $spot_id
            $status_terms = get_the_terms($spot_id, 'status'); // Змінено $spot->ID на $spot_id
            ...
            $final_parking_items[] = [
                'id' => $spot_id, // Змінено $spot->ID на $spot_id
                'type' => 'parking_space',
...
function fok_register_settings() {
    ...
    add_settings_field('fok_sales_phone', __('Телефон відділу продажів', 'okbi-apartments'), 'fok_render_sales_phone_field', 'flat_okbi_settings', 'fok_main_settings_section');
...
function fok_render_sales_phone_field() {
    $options = get_option( 'fok_global_settings' );
    $phone = $options['sales_phone'] ?? '';
    echo '<input type="tel" name="fok_global_settings[sales_phone]" value="' . esc_attr( $phone ) . '" class="regular-text" placeholder="+38 (0XX) XXX-XX-XX" />';
    echo '<p class="description">' . __('Цей номер буде відображено у хедері каталогу.', 'okbi-apartments') . '</p>';
}
...
function fok_sanitize_settings( $input ) {
    ...
    if ( isset( $input['sales_phone'] ) ) { $new_input['sales_phone'] = sanitize_text_field( $input['sales_phone'] ); }
...
/**
 * Отримує список всіх житлових комплексів з кешуванням.
 *
 * Використовує WordPress Transients API для кешування результату
 * на 1 годину, щоб зменшити кількість запитів до бази даних.
 *
 * @return array Масив об'єктів WP_Post для кожного ЖК.
 */
function fok_get_all_rcs_cached() {
    // Назва для нашого кешу
    $transient_name = 'fok_all_rcs_list';

    // Спробуємо отримати дані з кешу
    $rcs = get_transient( $transient_name );

    // Якщо в кеші нічого немає (або кеш застарів)
    if ( false === $rcs ) {
        ...
        // Зберігаємо результат в кеш на 1 годину (3600 секунд)
...
/**
 * Очищує кеш списку ЖК при оновленні/створенні/видаленні ЖК.
 *
 * @param int $post_id ID поста, що оновлюється.
 */
function fok_clear_rc_cache_on_save( $post_id ) {
    if ( get_post_type( $post_id ) === 'residential_complex' ) {
        delete_transient( 'fok_all_rcs_list' );
    }
}
add_action( 'save_post', 'fok_clear_rc_cache_on_save' );
add_action( 'delete_post', 'fok_clear_rc_cache_on_save' );

/**
 * Отримує вміст SVG-іконки з папки /assets/icons/
 *
 * @param string $icon_name Назва файлу іконки без розширення .svg.
 * @return string Вміст SVG-файлу або порожній рядок.
 */
function fok_get_icon( $icon_name ) {
