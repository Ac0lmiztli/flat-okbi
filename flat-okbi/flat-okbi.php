 ...
 * Збирає дані про секції та об'єкти для побудови координатної сітки на фронтенді.
 */
function fok_filter_properties_ajax_handler() {
    ...
    // 1. Отримуємо всі секції, що належать до цього ЖК.
    ...
    if ($sections_query->have_posts()) {
        while ($sections_query->have_posts()) {
            ...
            // Для кожної секції збираємо її параметри сітки.
            $sections_data[$section_id] = [
                'id'             => $section_id,
                'name'           => get_the_title(),
                'grid_rows'      => (int)get_post_meta($section_id, 'fok_section_total_floors', true),
                'grid_columns'   => (int)get_post_meta($section_id, 'fok_section_grid_columns', true),
                'properties'     => [], // Ініціалізуємо порожній масив для об'єктів
            ];
        }
    }
    wp_reset_postdata();

    // 2. Отримуємо всі об'єкти нерухомості, що належать до цього ЖК.
    $property_types = ['apartment', 'commercial_property', 'parking_space', 'storeroom'];
    $args = [
        'post_type' => $property_types,
        'posts_per_page' => -1,
        'meta_query' => [
            ['key' => 'fok_property_rc_link', 'value' => $rc_id]
        ],
        'orderby' => 'ID',
        'order' => 'ASC',
    ];
    ...
    if ($query->have_posts()) {
        while ($query->have_posts()) {
            ...
            // Якщо об'єкт не прив'язаний до секції або секції не існує, пропускаємо його.
            ...
            // 3. Збираємо всі необхідні дані про об'єкт, включаючи нові координати.
            $property_data = [
                'id'           => $property_id,
                'type'         => $post_type,
                'area'         => (float) get_post_meta($property_id, 'fok_property_area', true),
                'floor'        => (int) get_post_meta($property_id, 'fok_property_floor', true), // Залишаємо для фільтрів
                'status'       => $status_slug,
                'rooms'        => ($post_type === 'apartment') ? (int)get_post_meta($property_id, 'fok_property_rooms', true) : 0,
                'has_discount' => $discount_percent > 0,
                
                // --- НОВІ ДАНІ ДЛЯ СІТКИ ---
                'grid_x_start' => (int) get_post_meta($property_id, 'fok_property_grid_column_start', true) ?: 1,
                'grid_y_start' => (int) get_post_meta($property_id, 'fok_property_floor', true),
                'grid_x_span'  => (int) get_post_meta($property_id, 'fok_property_grid_column_span', true) ?: 1,
                'grid_y_span'  => (int) get_post_meta($property_id, 'fok_property_grid_row_span', true) ?: 1,
            ];
            
            // 4. Додаємо об'єкт до масиву 'properties' відповідної секції.
            $sections_data[$section_id]['properties'][] = $property_data;
        }
    }
    wp_reset_postdata();
    
    // Стара логіка групування видалена, оскільки вона більше не потрібна.
...
function fok_get_property_details_ajax_handler() {
    ...
    if ($post_type === 'apartment') {
        ...
        // Додамо рівневість для відображення в деталях
