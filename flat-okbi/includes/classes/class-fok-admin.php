...
class FOK_Admin {
    ...
    public function __construct() {
        ...
        add_action('wp_ajax_fok_delete_all_data', [$this, 'ajax_delete_all_data']);
    ...
    public function enqueue_admin_scripts($hook_suffix) {
        ...
        if ($hook_suffix !== 'flat-okbi_page_fok_settings') {
            return;
        }
        ...
        wp_add_inline_script('media-editor', '(function($){
            $(document).ready(function() {
                var mediaUploader;

                $("#fok-upload-logo-button").click(function(e) {
                    e.preventDefault();
                    if (mediaUploader) {
                        mediaUploader.open();
                        return;
                    ...
                    mediaUploader = wp.media.frames.file_frame = wp.media({
                        title: "' . __('Виберіть логотип', 'okbi-apartments') . '",
                        button: {
                            text: "' . __('Використати цей логотип', 'okbi-apartments') . '"
                        },
                        multiple: false
                    });
                    ...
                    mediaUploader.on("select", function() {
                        ...
                        $("#fok_logo_id").val(attachment.id);
                        $("#fok-logo-preview").html(\'<img src="\' + attachment.url + \'" style="max-width:150px;height:auto;">\');
                        $("#fok-remove-logo-button").show();
                    });

                    mediaUploader.open();
                });

                $("#fok-remove-logo-button").click(function(e) {
                    e.preventDefault();
                    $("#fok_logo_id").val("");
                    $("#fok-logo-preview").html("");
                    $(this).hide();
                });
            });
        })(jQuery);');
    ...
    public function handle_csv_export() {
        ...
        foreach ($properties as $property) {
            ...
            $rows[] = [
                'unique_id' => get_post_meta($property_id, 'fok_property_unique_id', true),
                'post_type' => $type_names[$post_type] ?? $post_type,
                'rc_name' => $rc_id_meta ? html_entity_decode(get_the_title($rc_id_meta)) : '',
                'section_name' => $section_id ? html_entity_decode(get_the_title($section_id)) : '',
                ...
                'grid_column_start' => get_post_meta($property_id, 'fok_property_grid_column_start', true),
                'grid_column_span' => get_post_meta($property_id, 'fok_property_grid_column_span', true),
                'grid_row_span' => get_post_meta($property_id, 'fok_property_grid_row_span', true),
                ...
                'price_per_sqm' => get_post_meta($property_id, 'fok_property_price_per_sqm', true),
                'total_price' => get_post_meta($property_id, 'fok_property_total_price_manual', true),
                'currency' => get_post_meta($property_id, 'fok_property_currency', true),
                'discount_percent' => get_post_meta($property_id, 'fok_property_discount_percent', true),
                'status' => $this->get_property_status_name($property_id),
                'layout_images' => implode(',', $image_filenames)
            ];
        }
    
        // 2. Natural order сортування: секція, потім поверх, потім номер
        usort($rows, function($a, $b) {
            $section_cmp = strnatcasecmp($a['section_name'], $b['section_name']);
            if ($section_cmp !== 0) return $section_cmp;
    
            $floor_cmp = strnatcasecmp($a['floor'], $b['floor']);
            if ($floor_cmp !== 0) return $floor_cmp;
    
            return strnatcasecmp($a['property_number'], $b['property_number']);
        });
    ...
    /**
     * AJAX handler to delete all plugin data.
     * This is a destructive action and should be used with caution.
     */
    public function ajax_delete_all_data() {
        // 1. Security check: Nonce and Capabilities
        if ( ! isset( $_POST['nonce'] ) || ! wp_verify_nonce( $_POST['nonce'], 'fok_delete_all_nonce' ) ) {
            wp_send_json_error( [ 'message' => __( 'Помилка безпеки (неправильний nonce).', 'okbi-apartments' ) ] );
        }
        if ( ! current_user_can( 'manage_options' ) ) {
            wp_send_json_error( [ 'message' => __( 'У вас недостатньо прав для виконання цієї дії.', 'okbi-apartments' ) ] );
        }

        // 2. Get all CPTs used by the plugin
        $post_types = ['residential_complex', 'section', 'apartment', 'commercial_property', 'parking_space', 'storeroom', 'fok_lead'];
        ...
        // We are not deleting terms or options here to avoid accidental data loss
        // if taxonomies/options are shared. This can be added if needed.

        wp_send_json_success( [ 'message' => sprintf( __( 'Усі дані плагіна (записи) успішно видалено. Видалено %d записів.', 'okbi-apartments' ), $posts_deleted ) ] );
    }
    ...
    public function handle_leads_export() {
        if (!isset($_POST['fok_leads_export_nonce']) || !wp_verify_nonce($_POST['fok_leads_export_nonce'], 'fok_leads_export_nonce_action')) {
