...
class FOK_Assets {
    ...
    public function enqueue_frontend_assets() {
        ...
        wp_enqueue_script('fok-custom-fix', plugin_dir_url( __FILE__ ) . '../../assets/js/custom-fix.js', ['fok-frontend-script'], time(), true);
    ...
    public function enqueue_admin_scripts( $hook_suffix ) {
        // --- Глобальні стилі для всіх сторінок нашого плагіна ---
        $current_screen = get_current_screen();
        $plugin_post_types = ['residential_complex', 'section', 'apartment', 'commercial_property', 'parking_space', 'storeroom', 'fok_lead'];
        $plugin_pages = ['toplevel_page_flat_okbi_settings', 'flat-okbi_page_fok_pricing', 'flat-okbi_page_fok_import', 'flat-okbi_page_fok_documentation'];

        if ( (isset($current_screen->post_type) && in_array($current_screen->post_type, $plugin_post_types)) || in_array($hook_suffix, $plugin_pages) ) {
            wp_enqueue_style( 'fok-admin-global-style', plugin_dir_url( __FILE__ ) . '../../assets/css/admin-global.css', [], time() );
        }
        ...
        if ( 'flat-okbi_page_fok_pricing' === $hook_suffix ) {
            ...
            wp_localize_script( 'fok-admin-pricing-script', 'fok_pricing_i10n', [
                ...
                'unsaved_changes_warning' => __( 'У вас є незбережені зміни. Якщо ви продовжите, вони будуть втрачені. Ви впевнені?', 'okbi-apartments' ),
            ]);
        }
