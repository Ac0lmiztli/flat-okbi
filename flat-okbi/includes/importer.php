    ...
    <div class="wrap">
        ...
        <div id="import-export-wrapper" style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
            <div class="card">
                ...
                <code>unique_id,post_type,rc_name,section_name,property_number,floor,grid_column_start,grid_column_span,grid_row_span,rooms,area,price_per_sqm,total_price,currency,discount_percent,status,layout_images</code>
...
function fok_handle_csv_upload() {
    ...
    foreach ($csv_data_rows as $row_data) {
        ...
        // --- ЗЧИТУВАННЯ НОВИХ ПОЛІВ КООРДИНАТ ---
        $grid_column_start = intval($row_data['grid_column_start'] ?? 1);
        $grid_column_span  = intval($row_data['grid_column_span'] ?? 1);
        $grid_row_span     = intval($row_data['grid_row_span'] ?? 1);
        ...
        // --- ЗБЕРЕЖЕННЯ НОВИХ ПОЛІВ КООРДИНАТ ---
        update_post_meta($property_id, 'fok_property_grid_column_start', $grid_column_start);
        update_post_meta($property_id, 'fok_property_grid_column_span', $grid_column_span);
        update_post_meta($property_id, 'fok_property_grid_row_span', $grid_row_span);
        ...
        if (!empty($image_filenames_str)) {
            ...
            foreach($image_paths as $path) {
                ...
                $attachment_id = filter_var($path, FILTER_VALIDATE_URL) ? attachment_url_to_postid($path) : fok_get_attachment_id_by_filename($path);
                if ($attachment_id) {
...
function fok_handle_csv_export() {
    ...
    foreach ($properties as $property) {
        ...
        // --- ОНОВЛЕНИЙ РЯДОК З НОВИМИ ДАНИМИ ---
        $row = [
