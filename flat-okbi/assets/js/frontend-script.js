jQuery(document).ready(function($) {
            ...
            sections.forEach(section => {
                ...
                if (regularProperties.length > 0 || parkingData.is_present) {
                    ...
                    if (regularProperties.length > 0) {
                        // --- ВИПРАВЛЕНА ЛОГІКА РОЗРАХУНКУ ПОВЕРХІВ ---
                        let minFloor = Infinity;
                        let maxFloor = -Infinity;

                        regularProperties.forEach(p => {
                            const startFloor = p.floor;
                            // Розраховуємо кінцевий поверх, враховуючи висоту об'єкта
                            const endFloor = p.floor + (p.grid_y_span || 1) - 1;
                            
                            if (startFloor < minFloor) {
                                minFloor = startFloor;
                            }
                            if (endFloor > maxFloor) {
                                maxFloor = endFloor;
                            }
                        });

                        if (minFloor === Infinity) { // На випадок, якщо немає об'єктів
                            minFloor = 0;
                            maxFloor = 0;
                        }
                        ...
                        html += '</div>'; // chessboard-grid
                    }

                    if (parkingData.is_present) {
                        html += `<div class="fok-parking-summary-block" data-parking-items='${JSON.stringify(parkingData.items)}'>
                                    <span class="fok-parking-summary-title">
                                        <span class="dashicons dashicons-car"></span>
                                        Паркінг
                                    </span>
                                    <span class="fok-parking-summary-stats">
                                        Вільно: <span class="available-count">${parkingData.available_count}</span> / ${parkingData.total_count}
                                    </span>
                                    <span class="fok-parking-summary-toggle dashicons dashicons-arrow-down-alt2"></span>
                                </div>
                                <div class="fok-parking-details-container"></div>`;
                    }
                    
                    html += `</div>`; // chessboard-grid-wrapper
                }
    ...
    function renderPropertyCell(property, maxFloor) {
        ...
        const gridStyles = isRegularProperty ? `
    ...
    function applyFilters() {
        ...
        $('.fok-apartment-cell').each(function() {
            ...
            // Замість .toggle() тепер використовуємо .toggleClass()
            $cell.toggleClass('is-filtered', !isVisible);
        });

        // Окремо обробляємо блок паркінгу
        $('.fok-parking-summary-block').each(function() {
            const $summaryBlock = $(this);
            const isParkingTypeEnabled = filters.types.includes('parking_space');
            
            // Додаємо/забираємо клас .is-filtered для всього блоку
            $summaryBlock.toggleClass('is-filtered', !isParkingTypeEnabled);

            // Якщо блок паркінгу відфільтровано, а він був відкритий - закриваємо його
            if (!isParkingTypeEnabled && $summaryBlock.hasClass('active')) {
                $summaryBlock.removeClass('active');
                $summaryBlock.next('.fok-parking-details-container').slideUp(400);
            }
        });
    }
    ...
    $resultsContainer.on('click', '.fok-parking-summary-block', function() { const $summaryBlock = $(this); const $detailsContainer = $summaryBlock.next('.fok-parking-details-container'); $summaryBlock.toggleClass('active'); if (!$detailsContainer.data('is-rendered')) { const items = $summaryBlock.data('parking-items'); let itemsHtml = ''; if (items && items.length > 0) { items.forEach(item => { itemsHtml += renderPropertyCell(item, null); }); } $detailsContainer.html(`<div class="fok-parking-grid">${itemsHtml}</div>`); $detailsContainer.data('is-rendered', true); applyFilters(); } $detailsContainer.slideToggle(400); });
