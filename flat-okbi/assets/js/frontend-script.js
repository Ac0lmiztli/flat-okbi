jQuery(document).ready(function($) {
    ...
    function renderPropertyCell(property, maxFloor) {
        ...
        let gridStyles = '';
        // ЗМІНА ТУТ: Додано перевірку "maxFloor !== null".
        // Якщо maxFloor дорівнює null, стилі позиціонування не будуть застосовані.
        if (property.grid_x_start && property.grid_x_start > 0 && maxFloor !== null) {
            ...
            const colNum = property.grid_x_start + 1;
    ...
    function showFloorPlanView(sectionId, floorNumber) {
        ...
        if (!section) {
            console.error("Помилка: Секцію не знайдено для ID:", sectionId);
            return;
        }

        const floorPlan = section.floor_plans.find(fp => fp.number == floorNumber);
        if (!floorPlan || !floorPlan.image) {
            alert('На жаль, план для цього поверху ще не завантажено.');
            return;
        }
    ...
    function openDetailsPanel(propertyId) {
        ...
        // ++ ЗМІНА: Додаємо підсвічування полігону при відкритті панелі ++
        const $polygons = $('#fok-plan-svg polygon');
        if ($polygons.length > 0) {
            $polygons.removeClass('is-active-on-plan');
            $polygons.filter(`[data-property-id="${propertyId}"]`).addClass('is-active-on-plan');
        }
    ...
    function closeDetailsPanel(force = false) {
        // ++ ЗМІНА: Прибираємо підсвічування з полігону при закритті панелі ++
    ...
    function renderPanelContent(data) {
        ...
        for (const [key, value] of Object.entries(data.params)) {
            if (value) {
                let buttonHtml = '';
                if (key === 'Поверх' && data.section_id) { // Додано перевірку data.section_id
                    buttonHtml = `<button class="fok-show-on-plan-btn" 
                                        data-section-id="${data.section_id}" 
                                        data-floor="${value}" 
                                        data-property-id="${data.id}" 
                                        title="Показати на плані поверху">
        ...
        if (data.status_slug === 'vilno' && data.base_price.trim() !== '0') {
            if (data.has_discount) {
                ...
                priceHtml = `<div class="fok-panel-price"><div class="total-price">${data.total_price} ${data.currency}</div><div class="price-per-m2">${data.price_per_m2} ${data.currency} / м²</div></div>`;
            }
        }
    ...
    $panelContent.on('submit', '#fok-booking-form', function(e) {
        ...
        const sendAjaxRequest = () => {
            $.ajax({
                ...
                success: (response) => {
                    ...
                    if (response.success) {
                        loadProperties();
                        setTimeout(() => closeDetailsPanel(true), 3000);
                    } else {
        ...
        // Перевіряємо, чи reCAPTCHA активна
        if (fok_ajax.recaptcha_site_key && typeof grecaptcha !== 'undefined') {
            grecaptcha.ready(function() {
                grecaptcha.execute(fok_ajax.recaptcha_site_key, { action: 'submit' }).then(function(token) {
                    // Додаємо токен до даних, що відправляються
                    submitData.recaptcha_token = token;
                    sendAjaxRequest();
                });
            });
        } else {
            // Якщо reCAPTCHA не налаштована, просто відправляємо форму
            sendAjaxRequest();
        }
    });
    ...
    $resultsContainer.on('click', '#fok-plan-svg polygon', function() {
        const $polygon = $(this);
        // Дозволяємо клікати тільки на вільні об'єкти
        if ($polygon.hasClass('status-vilno')) {
            const propertyId = $polygon.data('property-id');
            if (propertyId) {
                // ++ ЗМІНА: Замість того, щоб керувати класом напряму,
                // ми просто викликаємо openDetailsPanel, яка тепер сама все зробить.
    ...
    $resultsContainer.on('click', '.fok-parking-summary-block', function() {
        ...
        if (!$detailsContainer.data('is-rendered')) {
            ...
            if (items && items.length > 0) {
                ...
                // 1. Отримуємо дані по поточній секції, щоб мати доступ до її планів
                ...
                sortedLevels.forEach(level => {
                    ...
                    // 2. Перевіряємо, чи існує план для поточного рівня паркінгу
                    const hasPlan = sectionData && sectionData.floor_plans.some(plan => plan.number == level && plan.image);
                    // 3. Якщо план є, створюємо HTML для кнопки
                    ...
                    // 4. Додаємо кнопку перед номером рівня
    ...
    $panelContent.on('click', '.fok-show-on-plan-btn', function() {
        ...
        if (!sectionId || !floor) {
            alert("Не вдалося визначити секцію або поверх для цього об'єкта.");
            return;
        }
    
        showFloorPlanView(sectionId, floor);

        // Підсвічуємо полігон після того, як план з'явиться
        setTimeout(() => {
    ...
    // Коли повертаємось до шахматки, знімаємо підсвітку
    $resultsContainer.on('click', '.fok-back-to-chessboard-btn', function() {
