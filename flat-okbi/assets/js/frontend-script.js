jQuery(document).ready(function($) {
    ...
    // --- Нова логіка керування станом ---
    const BASE_URL_PATH = new URL(fok_ajax.viewer_page_url || window.location.href).pathname.replace(/\/property\/\d+\/?$/, '');
    ...
    // --- Нова централізована функція для синхронізації UI ---
    function syncUI() {
        const state = history.state || {};
        const propertyIdFromUrl = getPropertyIdFromUrl();

        if (propertyIdFromUrl) {
            targetPropertyId = propertyIdFromUrl;
            _showDetailsPanelUI(); // Ця функція тепер також закриває сайдбар
        } else if (state.filters === 'open') {
            _closeDetailsPanelUI(true);
            _showSidebarUI();
        } else {
            // Базовий стан - все закрито
            _closeDetailsPanelUI(true);
            _closeSidebarUI();
        }
    }
    ...
    function openPropertyDetails(propertyId) {
        ...
        const newUrl = `${BASE_URL_PATH}/property/${propertyId}/`.replace('//', '/');
        // Не міняємо стан, якщо URL вже правильний (напр. при popstate)
        if (window.location.pathname !== newUrl) {
             history.pushState({ propertyId: propertyId }, '', newUrl);
        }
        syncUI();
    }

    function closePropertyDetails() {
        history.pushState({ propertyId: null }, '', BASE_URL_PATH);
        syncUI();
    }
        ...
        xhr = $.ajax({
            ...
            success: (response) => {
                if (response.success) {
                    ...
                    applyFilters();
                    initScrollHints();
                    // Відкриваємо панель, якщо ID є в URL при завантаженні
                    const propertyIdFromUrl = getPropertyIdFromUrl();
                    if (propertyIdFromUrl) {
                        openPropertyDetails(propertyIdFromUrl);
                    }
                } else {
    ...
    function applyFilters() { 
        ...
        $('.fok-apartment-cell').each(function() { 
            ...
            if (filters.rooms) { // Перевіряємо, чи фільтр по кімнатах взагалі активний
                if (data.type === 'apartment') { 
                    if (filters.rooms === '3+' && data.rooms < 3) {
                        isVisible = false;
                    } 
                    if (filters.rooms !== '3+' && String(data.rooms) !== String(filters.rooms)) {
                        isVisible = false;
                    }
                } else {
                    // Якщо це не квартира, а фільтр по кімнатах активний - ховаємо
                    isVisible = false; 
                }
            }
    ...
    function renderPropertyCell(property, maxFloor) {
        ...
        const numberContent = ''; // Тепер номер ніколи не виводиться
    ...
    function _showViewerUI() {
        ...
        // Завантажуємо ЖК з кнопки, якщо він ще не встановлений
        if (!currentRCId) {
            currentRCId = $('.fok-open-viewer').data('rc-id');
        }
    ...
    function _showDetailsPanelUI() {
        ...
        _closeSidebarUI(); // ВАЖЛИВО: робимо панелі взаємовиключними
    ...
    $closeButton.on('click', () => {
        _hideViewerUI();
        history.pushState(null, '', BASE_URL_PATH);
    });
    ...
    $('#fok-mobile-filter-trigger').on('click', () => {
        history.pushState({ filters: 'open' }, '', BASE_URL_PATH);
        syncUI();
    });
    ...
    $(document).on('keydown', function(e) {
        ...
        if (e.key === "Escape") {
            ...
            } else if ($detailsPanel.hasClass('is-open') || $sidebar.hasClass('is-open')) {
                history.back();
            } else {
    ...
    $filtersForm.on('input change', 'input[type="number"], input[type="checkbox"], select', function() {
        // Окремий обробник для інпутів, крім кімнат
        debouncedApplyFilters();
    });

    $filtersForm.on('click', '.room-btn', function() {
        const $this = $(this);
        if ($this.parent().hasClass('fok-disabled')) return;
        $this.siblings().removeClass('active');
        $this.addClass('active');
        $('#filter-rooms').val($this.data('value'));
        debouncedApplyFilters();
    });

    $propertyTypeCheckboxes.on('change', function() {
    ...
    $lightbox.on('click', function(e) {
        // Close if the overlay itself or the close button (or its child SVG) is clicked
        if (e.target === this || $(e.target).closest('#fok-lightbox-close').length) {
            $lightbox.removeClass('is-open');
        }
    });
    ...
    function updateRoomFilterVisibility() {
        ...
        const $roomButtons = $roomFilter.find('.room-btn');
        const $roomInput = $('#filter-rooms');
        
        if (isApartmentChecked) {
            $roomFilter.removeClass('fok-disabled');
        } else {
            $roomFilter.addClass('fok-disabled');
            // Якщо фільтр по кімнатах був активний, скидаємо його
            if ($roomInput.val() !== '') {
                $roomInput.val('');
                $roomButtons.removeClass('active');
                $roomButtons.filter('[data-value=""]').addClass('active');
                // Нам потрібно викликати applyFilters вручну, оскільки ми не симулюємо подію change
                applyFilters(); 
            }
        }
    }
    ...
    function loadPropertyDetails(propertyId) {
        $panelContent.stop().css('opacity', 0); // Негайно ховаємо старий контент і зупиняємо будь-яну анімацію
    ...
    function renderPanelContent(data) {
        ...
        if (data.gallery && data.gallery.length > 0) {
            ...
            galleryHtml = `<div class="fok-panel-no-gallery">
                <svg viewBox="0 0 1024 1024" class="icon" version="1.1" xmlns="http://www.w3.org/2000/svg" fill="#000000"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"><path d="M658.461676 324.594173V95.397443h-270.868862v41.672133h229.19673V512.118771h-83.344266v41.672132h208.360664v-41.672132h-83.344266v-145.852465h229.19673v520.901659h-229.19673v-187.524597h-41.672132v187.524597h-479.229527v-333.377062h145.852465v-41.672132h-145.852465v-375.049195h145.852465v-41.672133h-187.524597v833.442655h833.442654v-604.245925z" fill="#54ad55"></path><path d="M825.150207 407.938439v416.721327h-166.688531v62.508199H887.658406v-479.229526zM554.281344 824.659766h-416.721327v62.508199h479.229527v-145.852464h-62.5082zM554.281344 178.741709h62.5082v333.377062h-62.5082z" fill="#bde5be"></path></g></svg>
                <p class="fok-panel-no-gallery-text">Планування очікується</p>
            </div>`;
        }
        let paramsHtml = '';
    ...
    function renderBookingForm(data) { 
        const formHtml = `<div class="fok-booking-form-wrapper">
            <button type="button" class="fok-form-back-btn">&larr; до об'єкту</button>
            <form id="fok-booking-form">
                <p>Заявка на ${data.type_name.toLowerCase()} №${data.property_number}</p>
                <div class="form-group">
                    <label for="b_name">Ваше ім'я</label>
                    <input type="text" id="b_name" class="fok-form-input" required>
                </div>
                <div class="form-group">
                    <label for="b_phone">Телефон</label>
                    <input type="tel" id="b_phone" class="fok-form-input" required>
                </div>
                <div id="booking-form-message"></div>
                <button type="submit" class="fok-booking-button fok-form-submit">${fok_i10n.send_request}</button>
            </form>
        </div>`; 
        $panelContent.animate({ opacity: 0 }, 200, function() { $(this).html(formHtml).animate({ opacity: 1 }, 250); }); 
    }
    ...
    // --- Нова функція для глибокого зв'язування ---
    function initDeepLinking() {
        const propertyId = getPropertyIdFromUrl();
        if (propertyId) {
            // Запитуємо контекст (ЖК) для цього об'єкта
            $.ajax({
                url: fok_ajax.ajax_url,
                type: 'POST',
                data: {
                    action: 'fok_get_context_for_property',
                    nonce: fok_ajax.nonce,
                    property_id: propertyId
                },
                success: function(response) {
                    if (response.success && response.data.rc_id) {
                        currentRCId = response.data.rc_id;
                        // Напряму викликаємо функції для відкриття, замість емуляції кліку
                        _showViewerUI();
                        loadProperties();
                    } else {
                        console.error('Deep linking error:', response.data.message);
                        history.replaceState({}, '', BASE_URL_PATH); // Скидаємо URL, бо об'єкт не знайдено
                    }
                },
                error: function() {
                    console.error('Deep linking AJAX error.');
                    history.replaceState({}, '', BASE_URL_PATH); // Скидаємо URL
                }
            });
        }
    }

    // Запускаємо логіку при завантаженні сторінки
    initDeepLinking();
});