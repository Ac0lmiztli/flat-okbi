jQuery(document).ready(function($) {
    ...
    function loadProperties() {
        ...
        // Повертаємо запит до початкового стану: він просто запитує всі об'єкти для ЖК
        xhr = $.ajax({
            ...
            success: (response) => {
                if (response.success) {
                    ...
                    // ++ ПОВЕРНУЛИ ВИКЛИК: Застосовуємо фільтри після завантаження ++
                    applyFilters();
                    initScrollHints();
                } else {
    ...
    // ++ ПОВЕРНУЛИ ФУНКЦІЮ: Логіка плавної фільтрації на клієнті ++
    ...
    // --- Обробники подій ---
    // ++ ПОВЕРНУЛИ СТАРУ ЛОГІКУ: Викликаємо applyFilters при змінах ++
    ...
    $resultsContainer.on('click', '.fok-parking-summary-block', function() {
        ...
        if (!$detailsContainer.data('is-rendered')) {
            ...
            // ++ ПОВЕРНУЛИ ВИКЛИК: Застосовуємо фільтри до щойно створених паркомісць ++
            applyFilters(); 
        }
    });
