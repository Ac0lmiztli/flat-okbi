jQuery(document).ready(function($) {
        ...
        if (sections && Object.keys(sections).length > 0) {
            ...
            for (const sectionId in sections) {
                ...
                html += `<div class="fok-section-block">
                            <h4>${section.name}</h4>
                            <div class="fok-floors-container">`;

                const existingFloors = Object.keys(section.floors).map(Number).sort((a, b) => a - b);

                existingFloors.forEach(floorNum => {
                    const floorData = section.floors[floorNum];
                    
                    html += `<div class="fok-floor-wrapper">
                                <div class="fok-floor-label">${floorNum}</div>
                                <div class="fok-floor-content">`;

                    if (floorData.is_grouped) {
                        const statusText = `Вільних: ${floorData.status_counts.vilno}`;
                        html += `
                            <div class="fok-grouped-row-wrapper">
                                <div class="fok-grouped-row-header" title="Натисніть, щоб розгорнути/згорнути">
                                    <span class="fok-group-title">${floorData.type_name}: ${floorData.count}</span>
                                    <span class="fok-group-status">${statusText}</span>
                                    <span class="fok-group-arrow"></span>
                                </div>
                                <div class="fok-grouped-items-container">`;
                        floorData.items.forEach(property => { html += renderPropertyCell(property); });
                        html += `   </div>
                            </div>`;
                    } else {
                        html += '<div class="fok-floor-row">';
                        floorData.forEach(property => {
                            html += renderPropertyCell(property);
                        });
                        html += '</div>';
                    }
    ...
    function renderPropertyCell(property) {
        let cellContent = '';
    ...
    $resultsContainer.on('click', '.fok-grouped-row-header', function() { $(this).closest('.fok-grouped-row-wrapper').toggleClass('is-expanded'); });
