jQuery(document).ready(function($) {
    ...
    function openFloorPlanEditor($row) {
        ...
        // Запит для отримання URL зображення (замість використання get-attachment)
        if (imageId) {
    ...
    mainContainer.on('change', '.fok-floor-number', function() {
        ...
        // Очищуємо кеш для цього поверху, щоб дані завантажились заново
    ...
    // Обробники подій всередині модального вікна
    ...
    modalSvg.on('mousedown', '.fok-polygon-point', function(e) {
        e.preventDefault();
        e.stopPropagation();
        isDrawing = false; // Вимикаємо малювання при перетягуванні точки
    ...
    // ++ ФІНАЛЬНА ВИПРАВЛЕНА ВЕРСІЯ ФУНКЦІЇ ++
    function stopPointDrag() {
        isDraggingPoint = false;
        draggedPointInfo = null;
        $(document).off('mousemove.dragPoint mouseup.dragPoint');
        
        // Повертаємо режим малювання після завершення перетягування
        if (activePropertyId) {
            isDrawing = true;
            modalImageContainer.css('cursor', 'crosshair'); // Повертаємо курсор-хрестик
        }
    }

    // Збереження всіх даних перед відправкою форми
