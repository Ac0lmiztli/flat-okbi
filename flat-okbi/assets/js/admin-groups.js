jQuery(document).ready(function($) {
    ...
    mainContainer.on('click', '.fok-upload-image-btn', function(e) {
        ...
        // Створюємо новий екземпляр медіа-вікна КОЖЕН раз при кліку.
        // Це гарантує, що контекст ($row) завжди буде правильним.
        ...
        // Обробник вибору зображення прив'язується до нового вікна
    ...
    modalImageContainer.on('click', function(e) {
        if (isDrawing && activePropertyId) {
            ...
            // --- Початок логіки "прилипання" (snapping) ---
            const snapThreshold = 0.015; // Чутливість прилипання (1.5% від ширини/висоти зображення)
            let snapped = false;

            // Перебираємо всі полігони і всі їхні точки
            for (const poly of polygonsData) {
                for (const point of poly.points) {
                    const distanceX = Math.abs(newX - point.x);
                    const distanceY = Math.abs(newY - point.y);

                    // Якщо курсор достатньо близько до існуючої точки
                    if (distanceX < snapThreshold && distanceY < snapThreshold) {
                        newX = point.x; // Прилипаємо до координат існуючої точки
                        newY = point.y;
                        snapped = true;
                        break; // Знайшли точку, виходимо з внутрішнього циклу
                    }
                }
                if (snapped) break; // Виходимо і з зовнішнього циклу
            }
            // --- Кінець логіки "прилипання" ---

            const polygon = getPolygonByPropertyId(activePropertyId);
            if (polygon) {
                polygon.points.push({ x: parseFloat(newX).toFixed(4), y: parseFloat(newY).toFixed(4) });
                renderSVG();
            }
        }
    });
