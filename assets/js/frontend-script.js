jQuery(document).ready(function($) {
    ...
    function loadPropertyDetails(propertyId) { $panelContent.animate({ opacity: 0 }, 200, function() { $panelLoader.show(); $.ajax({ url: fok_ajax.ajax_url, type: 'POST', data: { action: 'fok_get_property_details', nonce: fok_ajax.nonce, property_id: propertyId }, success: (response) => { $panelLoader.hide(); if (response.success) { lastFetchedPropertyData = response.data; currentGallery = response.data.gallery || []; renderPanelContent(response.data); } else { renderError(fok_i10n.data_loading_error); } }, error: () => { $panelLoader.hide(); renderError(fok_i10n.connection_error); } }); }); }
    ...
    // --- Початок нового блоку: Ефект підсвічування поверху при наведенні (v2) ---
    $resultsContainer.on('mouseenter', '.fok-apartment-cell', function() {
        const $cell = $(this);
        if ($cell.data('type') === 'parking_space') {
            return; 
        }
        const $grid = $cell.closest('.fok-chessboard-grid');

        // Отримуємо початковий рядок
        const startRow = parseInt($cell.css('grid-row-start'), 10);

        // Отримуємо висоту клітинки в рядках (наприклад, "span 2") і витягуємо з неї число
        ...
        // Підсвічуємо всі поверхи, які займає клітинка
        for (let i = 0; i < rowSpan; i++) {
            const currentRow = startRow + i;
            $grid.find('.fok-floor-label').filter(function() {
                return parseInt($(this).css('grid-row-start'), 10) === currentRow;
            }).addClass('is-highlighted');
        }

    }).on('mouseleave', '.fok-apartment-cell', function() {
        // Логіка для зняття виділення залишається такою ж
        $('.fok-floor-label.is-highlighted').removeClass('is-highlighted');
    });
});