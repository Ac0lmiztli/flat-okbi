jQuery(document).ready(function($) {
    ...
    $resultsContainer.on('click', '.fok-apartment-cell', function(e) {
        ...
        if (window.location.hash === '#details') {
            if (!lastFetchedPropertyData || lastFetchedPropertyData.id != propertyId) {
                _showDetailsPanelUI(); // Викликаємо функцію напряму
            }
        } else {
    ...
    $panelContent.on('click', '.fok-show-on-plan-btn', function() {
        ...
        // Перевіряємо, чи ми в мобільному режимі (де панель є оверлеєм)
        if ($detailsPanel.css('position') === 'fixed') {
            // Змінюємо хеш, щоб викликати стандартну логіку закриття панелі
            window.location.hash = 'viewer';
        }
        ...
        // Виділяємо потрібний полігон на плані з невеликою затримкою,
        // щоб гарантувати, що SVG вже відрендерився
        setTimeout(() => {
    ...
    function renderPanelContent(data) {
        ...
        for (const [key, value] of Object.entries(data.params)) {
            if (value) {
                ...
                // Показуємо кнопку, тільки якщо це поле "Поверх" І для нього є план
    ...
    /**
     * Ця функція забезпечує відповідність фільтрів, примусово видаляючи
     * чекбокс "Паркомісця", який не повинен відображатись.
     */
