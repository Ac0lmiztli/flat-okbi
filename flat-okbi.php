...
function fok_enqueue_admin_scripts( $hook_suffix ) {
    ...
    if ( 'toplevel_page_flat_okbi_settings' === $hook_suffix ) {
        ...
        wp_enqueue_media();
        wp_enqueue_script('fok-admin-script', plugin_dir_url( __FILE__ ) . 'assets/js/admin-settings.js', array( 'wp-color-picker', 'jquery' ), '2.2.5', true);
    }
    
    if ( 'flat-okbi_page_flat_okbi_import' === $hook_suffix ) {
        wp_enqueue_script('fok-admin-importer-script', plugin_dir_url(__FILE__) . 'assets/js/admin-importer.js', ['jquery'], '2.2.5', true);
        wp_enqueue_style('wp-color-picker');
        wp_enqueue_media();
        wp_enqueue_script('fok-admin-settings-script', plugin_dir_url( __FILE__ ) . 'assets/js/admin-settings.js', ['jquery', 'wp-color-picker', 'wp-mediaelement'], '2.2.5', true);
    ...
    if ( 'section' === $current_screen->post_type ) {
        ...
        wp_enqueue_style( 'fok-admin-grid-editor-style', plugin_dir_url( __FILE__ ) . 'assets/css/admin-grid-editor.css', [], '2.2.5' );
        wp_enqueue_script( 'fok-admin-grid-editor-script', plugin_dir_url( __FILE__ ) . 'assets/js/admin-grid-editor.js', ['jquery', 'jquery-ui-draggable', 'jquery-ui-droppable'], '2.2.5', true );
        ...
        wp_enqueue_style( 'fok-admin-floor-plan-editor-style', plugin_dir_url( __FILE__ ) . 'assets/css/admin-floor-plan-editor.css', [], '2.2.5' );
        wp_enqueue_script( 'fok-admin-groups-script', plugin_dir_url( __FILE__ ) . 'assets/js/admin-groups.js', ['jquery', 'jquery-ui-sortable', 'jquery-ui-dialog'], '2.2.5', true );
    ...
    if ( in_array( $current_screen->post_type, $property_post_types ) ) {
        wp_enqueue_script( 'fok-admin-logic', plugin_dir_url( __FILE__ ) . 'assets/js/admin-logic.js', ['jquery', 'select2'], '2.2.5', true );
    }
    
    // 4. Скрипти для сторінки "Житлові комплекси"
    if ( 'residential_complex' === $current_screen->post_type ) {
        wp_enqueue_script( 'fok-admin-rc-page-script', plugin_dir_url( __FILE__ ) . 'assets/js/admin-rc-page.js', ['jquery'], '2.2.5', true );
    }
}
